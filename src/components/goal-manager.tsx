...
const goalMeta: Record<GoalType, { label: string; description: string; unit: string; periodType: "WEEKLY" | "ONGOING"; defaultTarget: number; step: number; min: number; max: number }> = {
  WEEKLY_WORKOUTS: {
...
export function GoalManager({ progress }: { progress: GoalProgress[] }) {
  ...
  const [targetEdits, setTargetEdits] = useState<Record<string, number>>(() =>
    Object.fromEntries(progress.map((item) => [item.goal.id, item.goal.targetValue])),
  );
  ...
  async function updateGoal(item: GoalProgress, targetValue: number, isActive: boolean) {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.message ?? "Target belum berhasil diperbarui.");
      router.refresh();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Terjadi kesalahan.");
    } finally {
    ...
    <div className="space-y-4">
      {activeProgress.length > 0 ? (
        <div className="goal-grid">
          {activeProgress.map((item) => {
            ...
            const editedTarget = targetEdits[item.goal.id] ?? item.goal.targetValue;
            return (
              <article key={item.goal.id} className="goal-card">
                ...
                <label className="field mt-4"><span>Ubah target ({item.goal.unit})</span><input type="number" min={meta.min} max={meta.max} step={meta.step} value={editedTarget} onChange={(event) => setTargetEdits((current) => ({ ...current, [item.goal.id]: Number(event.target.value) }))} /></label>
                <div className="mt-3 flex flex-wrap gap-2">
                  <button type="button" className="goal-action" disabled={busy === item.goal.id || editedTarget === item.goal.targetValue} onClick={() => updateGoal(item, editedTarget, true)}>Simpan nilai</button>
                  <button type="button" className="goal-action" disabled={busy === item.goal.id} onClick={() => updateGoal(item, item.goal.targetValue, false)}>Nonaktifkan</button>
      ...
      <section className="form-card">
        ...
        <div className="form-grid">
          ...
          <label className="field"><span>Nilai target ({goalMeta[type].unit})</span><input type="number" min={goalMeta[type].min} max={goalMeta[type].max} step={goalMeta[type].step} value={target} onChange={(event) => setTarget(Number(event.target.value))} /></label>
        </div>
